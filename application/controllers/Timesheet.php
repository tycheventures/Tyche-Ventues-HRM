...
class Timesheet extends MY_Controller {
	...
	public function showholidays() {
		...
		$data['path_url'] = 'holidays';
			if(!empty($session)){ 
			$data['subview'] = $this->load->view("timesheet/showholidays", $data, TRUE);
			$this->load->view('layout_main', $data); //page load
			} else {
				redirect('');
			}		  
...
//print_r($result);
//echo '<hr>';
//echo $result[0]->employee_id;

		$result2 = $this->Employees_model->read_emp_leave_information($result[0]->employee_id);
		if(!$result2){$result2[0]->casual_leave=0;$result2[0]->medical_leave=0;}
		//print_r($result2);die();
//echo '<hr>';
		// get leave types
...
//		die();
		$data = array(
				...
				'emp_casual_leave' => $result2[0]->casual_leave,
				'emp_medical_leave' => $result2[0]->medical_leave,
...
$data[] = array(
				'<span data-toggle="tooltip" data-placement="top" title="View"><button type="button" class="btn btn-secondary btn-sm m-b-0-0 waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-holiday_id="'. $r->holiday_id . '"><i class="fa fa-eye"></i></button></span>',
				$r->event_name,
				$publish,
				$sdate,
				$edate
	 ...
	 public function leave_list() {
		...
		$data = array();
          ...
          foreach($leave->result() as $r) {
			 ...
			 $xUsr= $user[0];
			 if($xUsr){$full_name = $xUsr->first_name.' '.$xUsr->last_name;}
